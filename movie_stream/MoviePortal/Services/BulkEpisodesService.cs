...
public class BulkEpisodesService(AppDbContext db)
{
    ...
    public async Task<(Movie movie, int count, int maxNumber)?> LoadMovieInfoAsync(int movieId, CancellationToken ct = default)
    {
        ...
        var count = await db.Episodes.AsNoTracking().CountAsync(e => e.MovieId == movieId, ct);
        var max = await db.Episodes.AsNoTracking().Where(e => e.MovieId == movieId).Select(e => (int?)e.Number).MaxAsync(ct) ?? 0;
    ...
    public async Task<List<PlanRow>> BuildPlanAsync(int movieId, int start, int count, string titlePattern, DateTime? releaseStart, int intervalDays, CancellationToken ct = default)
    {
        ...
        var existed = await db.Episodes.AsNoTracking()
            .Where(e => e.MovieId == movieId && numbers.Contains(e.Number))
            .Select(e => new { e.Id, e.Number }).ToListAsync(ct);
        ...
        CancellationToken ct = default)
    {
        ...
        {
            if (p.Exists && !overwrite) { skipped++; continue; }

            Episode ep;
            if (p.Exists && p.EpisodeId is not null)
            {
                ep = await db.Episodes.FirstAsync(e => e.Id == p.EpisodeId.Value, ct);
                updated++;
            }
            else
            {
                ep = new Episode { MovieId = movieId, Number = p.Number };
                db.Episodes.Add(ep);
                created++;
            }
