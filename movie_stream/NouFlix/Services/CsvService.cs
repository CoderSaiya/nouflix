...
public class CsvService(IUnitOfWork uow)
{
    public async Task<List<SystemDto.EpisodeCsvPreviewRow>> BuildPreviewAsync(string csvText, CancellationToken ct = default)
    {
        ...
        var bySlug = slugs.Count == 0 ?
            new Dictionary<string, Movie>() :
            await uow.Movies
                .Query(false)
        ...
        var byTitle = titles.Count== 0 ?
            ...
            await uow.Movies
                ...
                .ToDictionaryAsync(m => m.Title, ct);
        ...
        {
            var nums = await uow.Episodes
                .Query()
                .Where(e => e.MovieId == mid).Select(e => e.Number)
                .ToListAsync(ct);
            existsMap[mid] = new HashSet<int>(nums);
        }

        var preview = new List<SystemDto.EpisodeCsvPreviewRow>();
        var dupChecker = new HashSet<(int movieId, int number)>();
        int rowIndex = 1;

        foreach (var r in rows)
        {
            var pr = new SystemDto.EpisodeCsvPreviewRow { RowNumber = rowIndex++ };
    ...
    public async Task<SystemDto.EpisodeCsvImportResult> ImportAsync(IEnumerable<SystemDto.EpisodeCsvPreviewRow> preview, bool overwrite, bool autoCreateSeason, CancellationToken ct = default)
    {
        ...
        foreach (var row in preview.Where(r => r.IsValid))
        {
            try
            {
                var ep = await uow.Episodes.GetByMovieAndNumberAsync(row.MovieId, row.Number, ct);
                ...
                if (row.SeasonNumber is not null)
                {
                    ...
                    {
                        var season = await uow.Seasons
                            .Query(false)
                            .FirstOrDefaultAsync(s => s.MovieId == row.MovieId && s.Number == row.SeasonNumber.Value, cancellationToken: ct);
        ...
        await uow.SaveChangesAsync(ct);
        return new SystemDto.EpisodeCsvImportResult(created, updated, skipped, failed);
    }

    // CSV parsing
    private List<SystemDto.EpisodeCsvRawRow> ParseCsv(string text)
    {
        ...
        var rows = new List<SystemDto.EpisodeCsvRawRow>();
        for (int idx = 1; idx < lines.Count; idx++)
        {
            var fields = ParseLine(lines[idx]);
            var r = new SystemDto.EpisodeCsvRawRow();
        ...
        static DateTime? ToDate(string? s)
        {
            ...
            if (DateTime.TryParseExact(s.Trim(), ["yyyy-MM-dd", "yyyy/M/d", "dd/MM/yyyy"], CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal, out var d))
                return d.Date;
            return null;
        }
